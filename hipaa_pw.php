...
foreach($userList as $user){
	...
	if($days_old > $HIPAA_trigger){
		...
		if($user["profile"]["mobilePhone"]!=""){
			$notify = sms_notify($user["id"],$user["profile"]["mobilePhone"]);
		}
	}
...
function sms_notify($userid,$phone_number){
	
	// Set our AccountSid and AuthToken
	$sid = '{USE YOUR SID} https://www.twilio.com/console';
	$token = 'USE YOUR TOKEN https://www.twilio.com/console';


	require_once('Services/Twilio.php');  //USE YOUR OWN REFERENCE
	
	$smsMsg = "Your LIGHTHOUSE password has been reset to meet our security procedures.";

 	$to_phone = "+1".$phone_number;

	$client = new Services_Twilio($sid, $token);
 
	$client->account->messages->sendMessage("USE ONE OF YOUR PURCHASED NUMBERS FOR CALLERID https://www.twilio.com/console/phone-numbers/incoming", $to_phone, $smsMsg);
